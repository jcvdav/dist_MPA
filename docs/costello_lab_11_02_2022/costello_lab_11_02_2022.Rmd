...
subtitle: "~~Costello~~ Gaines Lab Meeting"  
author: "J.C."
date: 'Feb 11, 2022'
output:
...
```{r}
knitr::include_graphics("img/map_old_revilla.png") 
```
...
```{r}
knitr::include_graphics("img/map_new_revilla.png") 
```
...
```{r, out.width=1000}
knitr::include_graphics("img/total_hours_before_map.png")
```
...
```{r}
knitr::include_graphics("img/most_unclassified_before.png")
```
...
```{r}
knitr::include_graphics("img/most_classified_before.png") 
```
...
```{r}
knitr::include_graphics("img/annual_pct_inside_outside.png") 
```
...
```{r}
knitr::include_graphics("img/annual_pct_inside_by_vessel.png") 
```
...
```{r}
knitr::include_graphics("img/distance_and_usage.png")
```
...
```{r}
knitr::include_graphics("img/percent_effort_inside_closest_vessels.png")
```
...
$$\Pi_{it} = \sum_{j = 1}^J \left(pq_{ij}E_{ijt}X_{jt} - cE_{ijt}^\beta\right)$$
...
$$\theta_{i + 1} = f(theta_{ijt}, \text{visitation}, \text{distance})$$
